...
- Instrumenting at a gateway instead
  ...
  covers every server behind it without touching any of them. It captures
  the arguments the model requested, which is a slightly different fact from
  the arguments a server received, and coverage varies by provider. Its
...
- [`cerberus-flex-gateway`](../cerberus-flex-gateway/README.md), the MuleSoft Flex Gateway policy — gateway-side coverage for MuleSoft deployments; fronting an MCP server behind it needs the server's DNS-rebinding host allow-list updated for the upstream name, see its README's "Fronting an MCP server"
