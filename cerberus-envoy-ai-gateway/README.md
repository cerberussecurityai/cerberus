...
| An unsampled upstream `traceparent` silently drops the request | The gateway's sampler is parent-based unless overridden, so an inbound `traceparent` ending `-00` makes it record **nothing** — no attributes, no export, no event. This is worse than sending no traceparent at all (an absent parent always samples). Set `OTEL_TRACES_SAMPLER=always_on` as shown below; it still inherits the upstream `trace_id` but guarantees capture. If your application samples traces at less than 100% and propagates context into the gateway, anything else will silently cost you that share of your AI traffic. |
