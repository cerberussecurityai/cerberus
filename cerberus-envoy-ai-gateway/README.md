...
| An unsampled upstream `traceparent` silently drops the request | The gateway's sampler is parent-based unless overridden, so an inbound `traceparent` ending `-00` makes it record **nothing** — no attributes, no export, no event. This is worse than sending no traceparent at all (an absent parent always samples). **Which requests this hits follows the same carrier split as the row above:** the LLM path reads the **HTTP header**, so a `-00` header drops LLM spans; the MCP path reads **only** `params._meta`, so a `-00` header leaves MCP spans untouched and only a `-00` in `_meta` drops them. Set `OTEL_TRACES_SAMPLER=always_on` as shown below; it restores capture on **both** paths and still inherits the upstream `trace_id`, so correlation is not the price of the fix. If your application samples traces at less than 100% and propagates context into the gateway, anything else will silently cost you that share of your AI traffic. Measured on aigw v0.7.0 and v1.0.0, identically on both: 3 requests per cell, every one dropped under the parent-based default and every one exported under `always_on`. That covers **sampling and trace-context behaviour only** — span *attribute* names are still verified against v0.7.0 alone (rows above), so this is not a statement that v1.0.0 is supported generally. To re-check on a version bump, one run of the playground shows it: `OTEL_TRACES_SAMPLER= ./run.sh` for the parent-based default, then `./drive.sh --traceparent 00-<32hex>-<16hex>-00`, which sends the context on both carriers. No events reaching the stub ingest is the drop; re-run without the sampler override and they appear. |
