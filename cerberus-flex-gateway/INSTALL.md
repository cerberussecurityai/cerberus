...
| An MCP server behind the gateway answers `421 Misdirected Request` to every request | Flex Gateway rewrites `Host` to the upstream address; the Python `mcp` SDK's DNS-rebinding protection (FastMCP 1.10+, `MCPServer` 2.x) only allows `localhost` / `127.0.0.1` / `[::1]` by default, and a direct health probe on the server's own port stays green. Give the server a `TransportSecuritySettings` whose `allowed_hosts` names the upstream address (and, for browser clients, whose `allowed_origins` names the sites those apps are served from — otherwise the 421 becomes a 403) — see [Fronting an MCP server](README.md#fronting-an-mcp-server). |
